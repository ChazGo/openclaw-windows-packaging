...
internal sealed class WindowsHostConsole : IHostConsole
{
    ...
    private sealed class ConsoleCapture : IDisposable
    {
        ...
        private readonly nint _vtOutput;
            ...
            Action<string> log)
        {
                ...
                native.GetStdHandle(StdOutput), native.GetStdHandle(StdError) })
            {
                ...
                if (_vtOutput == 0 && (mode & EnableVirtualTerminalProcessing) != 0)
                {
                    _vtOutput = output;
                }
            }
        ...
        public void Dispose()
        {
            ...
            if (_vtOutput != 0)
            {
                try
                {
                    ...
                    _native.WriteConsole(_vtOutput, ResetSequence);
                }
            ...
            foreach ((nint handle, uint mode) in _outputModes)
            {
                TryRestore(
                    ...
                    () =>
                    {
                        _native.GetConsoleMode(handle, out uint current);
                        _native.SetConsoleMode(
                            handle,
                            mode | (current & EnableVirtualTerminalProcessing));
                    });
            }
