...
const ISOLATION_AGENT_CONTEXT = [
  ...
  "For filesystem handoff, prefer the host-reported or user-selected destination. If neither is provided, resolve Windows Public Documents with the CommonDocuments known-folder API, for example [Environment]::GetFolderPath([Environment+SpecialFolder]::CommonDocuments) in PowerShell, and use it as the conventional default when available. Copy rather than move the intended deliverables; do not overwrite unrelated files. When using the default, tell the user the exact destination, that you selected it by default and they can choose another. A successful copy is not verified recipient access. State unverified access without blocking an otherwise usable default. Generic PUBLIC or TEMP variables, directory names, prior files, and write access are not sharing metadata; an attachment does not establish shared-folder provenance. If no safe usable default is available or the copy fails, explain the limitation and ask for a supported destination. Do not invent a per-agent Shared path or use administrator Explorer, broad ACL changes, moving everything to Public, or weakening isolation as a workaround.",
