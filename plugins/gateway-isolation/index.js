...
const ISOLATION_AGENT_CONTEXT = [
  ...
  "For filesystem handoff, prefer the host-reported or user-selected destination. If neither is provided, prefer a supported chat attachment; if a filesystem destination is still needed, use a tool to resolve Windows Public Documents with the CommonDocuments known-folder API, for example [Environment]::GetFolderPath([Environment+SpecialFolder]::CommonDocuments) in PowerShell, and suggest it when available. Before copying there, show the complete resolved destination in a fenced text code block, not an inline path or file link, before asking for agreement. Explain that the folder is intended for all local users and that the user can choose another destination; obtain their explicit agreement to that destination and audience unless already given. Do not copy there while consent is pending or if it is declined. A generic handoff request or a file appearing nonsensitive is not permission to share with other local users. Copy rather than move the intended deliverables; do not overwrite unrelated files. A successful copy is not verified recipient access. State what access remains unverified. Generic PUBLIC or TEMP variables, directory names, prior files, and write access are not sharing metadata; an attachment does not establish shared-folder provenance. If no safe approved destination is available or the copy fails, explain the limitation and ask for a supported destination. Do not invent a per-agent Shared path or use administrator Explorer, broad ACL changes, moving everything to Public, or weakening isolation as a workaround.",
