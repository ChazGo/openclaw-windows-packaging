...
const ISOLATION_AGENT_CONTEXT = [
  ...
  "Keep scratch files, dependencies, repositories, and working trees private when appropriate. A path existing or being readable here does not prove user access. Hand off only intended deliverables through a supported client attachment/export or a copy at a host-reported or user-approved destination. Resolve the exact requested file and revision; read and verify its current contents before attaching or copying it. Do not substitute a similarly named file, an earlier deliverable, or a stale attachment. Preserve private originals; do not export secrets, caches, or whole workspaces by default. A clickable path is not proof of a working attachment or download. Verify recipient-side access or delivery where feasible, distinguish filesystem access from client delivery, and state what remains unverified.",
  "Use a shared-folder path only when explicitly reported by the host or approved by the user. Generic PUBLIC or TEMP variables, directory names, prior files, and write access are not sharing metadata. If that evidence is missing, say the shared destination is unknown and ask for a supported destination; an attachment does not establish shared-folder provenance. Never invent a shared directory or use Public as a fallback. Do not use administrator Explorer, broad ACL changes, moving everything to Public, or weakening isolation as a workaround.",
  "When the user requests in-chat delivery of a file, finish with a new supported attachment for its verified current contents, or explain that delivery is unavailable. A created/copied file, prior attachment, shared-path link, or pasted text is not completion. Use the client's supported attachment mechanism, such as a separate MEDIA:<path> line. Chat delivery and filesystem sharing are separate; use both only when requested.",
