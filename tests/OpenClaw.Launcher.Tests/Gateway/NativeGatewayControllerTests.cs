...
public sealed class NativeGatewayControllerTests : IDisposable
{
    ...
    private string WorkingDirectory => Path.Combine(_root, "profile");
    ...
    public async Task HappyStartStatusAndStopPreserveVerifiedOwnership()
    {
        ...
        NativeGatewayLifecycleSource source =
            Assert.IsType<NativeGatewayLifecycleSource>(status.Source);
        Assert.Equivalent(evidence, source.Record, strict: true);
        Assert.Equal(LogPath, source.LogPath);
        ...
        Assert.Equal(WorkingDirectory, evidence.WorkingDirectory);
