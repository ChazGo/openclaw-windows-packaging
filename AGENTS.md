...
- An explicit `developmentCommit` may own both workflow defaults, but it never authorizes official signing. Official runs must explicitly select `approvedCommit`; development-runtime qualification is not proof of officially signed production readiness.
